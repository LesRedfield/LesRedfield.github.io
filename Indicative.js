...
(function(root, factory) {
  ...
  var DEBUG = true;
  var ENDPOINT = "https://api.indicative.com/service";
  ...
  Indicative.prototype.linkClick = function(event, linkName) {
    ...
    this.buildEvent("Link Click", linkProps, function () {
       if (url) {
         window.location = url;
       }
    });
  ...
  Indicative.prototype.formSubmit = function(event) {
    ...
    var formProps = this._buildFormProps(event.target);
    event.preventDefault();

    this.buildEvent("Form Submit", formProps, function(e) {
      e.target.submit();
    }.bind(this, event));
  };
  ...
  Indicative.prototype._trackLinks = function() {
    ...
    for (var idx = 0; idx < links.length; idx++) {
      links[idx].addEventListener("click", this.linkClick.bind(this));
    }
  };
  ...
  Indicative.prototype._trackForms = function() {
    ...
    for (var idx = 0; idx < forms.length; idx++) {
      forms[idx].addEventListener("submit", this.formSubmit.bind(this));
    }
  };
  ...
  Indicative.prototype._buildFormProps = function(form) {
    ...
    var formAttributes = ["id", "name"];

    for (var index in formAttributes) {
      var attribute = formAttributes[index];
      if (form[attribute]) {
        formProps.form[attribute] = form[attribute];
      }
    }
    ...
    for (var tagIndex in inputTags) {
      var tag = inputTags[tagIndex];
      var inputList = form.getElementsByTagName(tag);
      for (var inputListIndex = 0; inputListIndex < inputList.length; inputListIndex++) {
        var inputElement = inputList[inputListIndex];
    ...
    var inputAttributes = ["name", "id", "value"];

    for (var inputIndex in inputs) {
      var input = inputs[inputIndex];
      var inputObj = {};
      var inputKey = inputIndex;
      ...
      for (var attributeIndex in inputAttributes) {
        var inputAttribute = inputAttributes[attributeIndex];
        if (input[inputAttribute]) {
          inputObj[inputAttribute] = input[inputAttribute];
        }
      }
